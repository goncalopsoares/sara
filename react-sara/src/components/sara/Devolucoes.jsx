    ...
    return (
        <>
            {porDevolverRecolher.map(req => {
                ...
                if (
                    justDate(req.data_fim_requisicao) === justDate(propsDate) &&
                    (req.id_estado === 4 || req.id_estado === 5)
                ) {
                    return (
                        <Link to={`/requisicao/${req.id_requisicao}`} key={req.id_requisicao} style={{ textDecoration: 'none', color: '#000' }}>
                            <div className={`${req.id_estado === 4 ? 'background-grey-300' : 'background-green-50'} p-4 mb-2`} style={{ borderRadius: '1rem' }}>
